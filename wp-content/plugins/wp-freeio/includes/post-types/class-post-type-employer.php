...
class WP_Freeio_Post_Type_Employer {

	public static $prefix = WP_FREEIO_EMPLOYER_PREFIX;
	...
	public static function register_post_type() {
		$singular = __( 'Employer', 'wp-freeio' );
		$plural   = __( 'Employers', 'wp-freeio' );
		...
		$has_archive = true;
		$employer_archive = get_option('wp_freeio_employer_archive_slug');
		if ( $employer_archive ) {
			$has_archive = $employer_archive;
		}
		$rewrite_slug = get_option('wp_freeio_employer_base_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'employer', 'Employer slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'       => $rewrite_slug,
			'with_front' => false
	...
	public static function save_field_employees($updated, $action, $obj) {
		if ( $action != 'disabled' ) {
			...
			$employee_users = WP_Freeio_Query::get_employee_users($post_id, array('fields' => 'ids'));
	...
	public static function metaboxes() {
		...
		if ( $pagenow == 'post.php' || $pagenow == 'post-new.php' ) {
			do_action('wp-freeio-employer-fields-admin');
		}
	}
	...
	public static function fields_front( array $metaboxes ) {
		if ( ! is_admin() ) {
			...
			$init_fields = apply_filters( 'wp-freeio-employer-fields-front', array(), $post_id );
			...
			$fields = apply_filters( 'wp-freeio-employer-fields', $fields, $post_id );
		...
		switch ( $column ) {
			...
			case 'location':
				...
				if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
					...
					echo sprintf( '<a href="?post_type=employer&location=%s">%s</a>', $location->slug, $location->name );
				} else {
					echo '-';
				}
				break;
...
WP_Freeio_Post_Type_Employer::init();
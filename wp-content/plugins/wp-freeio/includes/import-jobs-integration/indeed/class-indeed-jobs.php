...
class WP_Freeio_Indeed_Jobs_Hooks {

    private static $key = 'wp_freeio_indeed_import';
    ...
    public static function init() {
        ...
        add_action( 'wp_ajax_wp_freeio_ajax_indeed_job_import', array( __CLASS__, 'process_import_indeed_jobs' ) );
    ...
    public static function indeed_jobs_admin_menu() {
        if ( wp_freeio_get_option('indeed_job_import_enable') || (isset($_POST['indeed_job_import_enable']) && $_POST['indeed_job_import_enable'] == 'on') ) {
            add_submenu_page( 'edit.php?post_type=job_listing', esc_html__('Import Indeed Jobs', 'wp-freeio'), esc_html__('Import Indeed Jobs', 'wp-freeio'), 'manage_options', 'import-indeed-jobs', array( __CLASS__, 'import_indeed_jobs_settings' ) );
        }
    }
    ...
    public static function import_indeed_jobs_settings() {
        ...
        <div class="wrap wp_freeio_settings_page cmb2_options_page">
            <h2><?php esc_html_e('Import Indeed Jobs', 'wp-freeio'); ?></h2>
    ...
    public static function import_indeed_fields() {
        $fields = array(
            'id'         => 'options_page',
            'wp_freeio_title' => __( 'Indeed Jobs Import', 'wp-freeio' ),
            'show_on'    => array( 'key' => 'options-page', 'value' => array( self::$key, ), ),
            'fields'     => apply_filters( 'wp_freeio_indeed_job_import_fields', array(
                    ...
                    array(
                        'name'    => __( 'Country', 'wp-freeio' ),
                        'desc'    => __( 'Select a country for search.', 'wp-freeio' ),
                        'id'      => 'indeed_job_import_country',
                        'type'    => 'select',
                        'options' => WP_Freeio_Indeed_API::indeed_api_countries()
                    ),
                    array(
                    ...
                    array(
                        ...
                        'desc'    => __( 'Choose which type of job to query.', 'wp-freeio' ),
                        'id'      => 'indeed_job_import_type',
                        'type'    => 'select',
                        'options' => WP_Freeio_Indeed_API::indeed_job_types()
                    ),
                    array(
                        'name'    => __( 'Sort By', 'wp-freeio' ),
                        'desc'    => __( 'Choose sort query results by Date/Relevance.', 'wp-freeio' ),
                        'id'      => 'indeed_job_import_sort_by',
                        'type'    => 'select',
                        'options' => array(
                            'date' => esc_html__('Date', 'wp-freeio'),
                            'relevance' => esc_html__('Relevance', 'wp-freeio'),
                        )
                    ),
                    array(
                        'name'    => __( 'Start Import Jobs', 'wp-freeio' ),
                        'desc'    => __( 'Enter start number to import jobs. Default start number is 1.', 'wp-freeio' ),
                        'id'      => 'indeed_job_import_start_number',
                        'type'    => 'text',
                        'default' => '1',
                    ),
                    array(
                        'name'    => __( 'Number of Jobs to Import (Maximum Limit 25)', 'wp-freeio' ),
                        'desc'    => __( 'Enter number of jobs to import. Default number of import jobs is 10. Maximum import jobs limit is 25.', 'wp-freeio' ),
            ...
            $metaboxes[ $prefix . 'indeed_job_fields' ] = array(
                ...
                'fields'                    => array(
                    array(
                        ...
                        'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_detail_url',
                        'type'              => 'text',
                    ),
                    array(
                        'name'              => __( 'Company Name', 'wp-freeio' ),
                        'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_company_name',
                        'type'              => 'text',
                    ),
                ),
            );
    ...
    public static function get_job_type($type) {
        switch ($type) {
            case 'fulltime' :
                $type = esc_html__('Full Time', 'wp-freeio');
                break;
            case 'parttime' :
                $type = esc_html__('Part Time', 'wp-freeio');
                break;
            case 'contract' :
                $type = esc_html__('Contract', 'wp-freeio');
                break;
            case 'internship' :
                $type = esc_html__('Internship', 'wp-freeio');
                break;
            case 'temporary' :
                $type = esc_html__('Temporary', 'wp-freeio');
                break;
        }
        return $type;
    }
    ...
    public static function process_import_indeed_jobs() {
        $publisher_number = wp_freeio_get_option('indeed_job_import_number', '');
        ...
        $indeed_jobs = WP_Freeio_Indeed_API::get_jobs($api_args);
        ...
        if (isset($indeed_jobs['error']) && $indeed_jobs['error'] != '') {
            ...
            foreach ($indeed_jobs as $indeed_job) {
                ...
                $existing_id = WP_Freeio_Mixes::get_post_id_by_meta_value( WP_FREEIO_JOB_LISTING_PREFIX.'indeed_detail_url', $indeed_job->url);
                ...
                if ( $post_author ) {
                    ...
                    $user_id = WP_Freeio_User::generate_user_by_post_name($indeed_job->company);
                    if ( $user_id ) {
                ...
                if ($indeed_job->country != '') {
                    ...
                    $countries = WP_Freeio_Indeed_API::indeed_api_countries();
                    $address[] = isset($countries[strtolower($indeed_country)]) ? $countries[strtolower($indeed_country)] : '';
                }
                if (!empty($address)) {
                    $address = implode(', ', $address);
                    $location_addrs['address'] = $address;
                    add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'address', $address, true);
                } else {
                    $location_addrs['address'] = '';
                }

                // Insert job latitude meta key
                add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'map_location_latitude', esc_attr($indeed_job->latitude), true);

                // Insert job longitude meta key
                add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'map_location_longitude', esc_attr($indeed_job->longitude), true);
                ...
                add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'job_referral', 'indeed', true);

                // Insert job detail url meta key
                add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_detail_url', esc_url($indeed_job->url), true);

                // Insert job comapny name meta key
                add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_company_name', $indeed_job->company, true);
            ...
            $json = array(
                'status' => false,
                'msg' => sprintf(__('%s indeed jobs are imported successfully.', 'wp-freeio'), count($indeed_jobs))
            );
            echo json_encode($json);
            die();
        }
    ...
    public static function get_company_name($ouput, $post) {
        ...
        $indeed_company_name = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_company_name', true);
    ...
    public static function get_company_name_html($ouput, $post) {
        ...
        $indeed_company_name = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_company_name', true);
        if ($job_referral == 'indeed' && $indeed_company_name != '') {
            $ouput = sprintf(wp_kses(__('<span class="employer text-theme">%s</span>', 'wp-freeio'), array( 'span' => array('class' => array()) ) ), $indeed_company_name );
        }
        return $ouput;
    }
    ...
    public static function view_more_btn($post) {
        $ouput = '';
        $job_referral = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'job_referral', true);
        $indeed_detail_url = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'indeed_detail_url', true);
        if ($job_referral == 'indeed' && $indeed_detail_url != '') {
            $ouput = '<div class="view-more-link"><a class="btn btn-theme" href="' . $indeed_detail_url . '">' . esc_html__('view more', 'wp-freeio') . '</a></div>';
        }
        return $ouput;
    }
}

add_filter( 'cmb2_get_metabox_form_format', 'wp_freeio_indeed_modify_cmb2_form_output', 10, 3 );

function wp_freeio_indeed_modify_cmb2_form_output( $form_format, $object_id, $cmb ) {
    if ( 'wp_freeio_indeed_import' == $object_id && 'options_page' == $cmb->cmb_id ) {

        return '<form class="cmb-form" method="post" id="%1$s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%2$s">%3$s<div class="wp_freeio-submit-wrap"><input type="button" name="submit-cmb-indeed-job-import" value="' . __( 'Import Indeed Jobs', 'wp-freeio' ) . '" class="button-primary"></div></form>';
    }

    return $form_format;
}

WP_Freeio_Indeed_Jobs_Hooks::init();
(function ($) {
    ...
    WJBPAjaxFileUpload.prototype = {
        init: function() {
            ...
            $('.wp-freeio-file-upload').each(function(){
            ...
            $(document).on('click', '.wp-freeio-uploaded-files .wp-freeio-remove-uploaded-file', function(e) {
                e.preventDefault();
                var $file_field = $(this).closest('.cmb-type-wp-freeio-file').find('input.wp-freeio-file-upload');
                ...
                $(this).closest('.wp-freeio-uploaded-file').remove();
            });
        },
        upload_file: function($element) {
            $element.fileupload({
                dataType: 'json',
                dropZone: $(this),
                url: wp_freeio_file_upload.ajax_url_endpoint.toString().replace( '%%endpoint%%', 'wp_freeio_ajax_upload_file' ),
                maxNumberOfFiles: 1,
                formData: {
                    script: true,
                    action: 'wp_freeio_ajax_upload_file'
                },
                add: function (e, data) {
                    var $file_field     = $( this );
                    var $form           = $file_field.closest( 'form' );
                    var $uploaded_files = $file_field.parent().find('.wp-freeio-uploaded-files');
                    var uploadErrors    = [];
                    ...
                    if ( typeof $file_field.data( 'file_limit_left' ) !== 'undefined' ) {
                        ...
                        var currentFiles = parseInt( $uploaded_files.children( '.wp-freeio-uploaded-file' ).length, 10);
                    ...
                    if ( allowed_types ) {
                        ...
                        if ( data.originalFiles[0].name.length && ! acceptFileTypes.test( data.originalFiles[0].name ) ) {
                            uploadErrors.push( wp_freeio_file_upload.i18n_invalid_file_type + ' ' + allowed_types );
                        }
                    }
                    ...
                    $.each(data.result.files, function(index, file) {
                        if ( file.error ) {
                            ...
                            if ( $.inArray( file.extension, image_types ) >= 0 ) {
                                html = $.parseHTML( wp_freeio_file_upload.js_field_html_img );
                                $( html ).find('.wp-freeio-uploaded-file-preview img').attr( 'src', file.url );
                                $( html ).find('.wp-freeio-uploaded-file-name').remove();
                            } else {
                                html = $.parseHTML( wp_freeio_file_upload.js_field_html );
                                ...
                                $( html ).find('.wp-freeio-uploaded-file-name .freelancer-detail-cv').html( new_html );

                                $( html ).find('.wp-freeio-uploaded-file-preview').remove();
                            }
    ...
    $.wpfiAjaxFileUpload = WJBPAjaxFileUpload.prototype;

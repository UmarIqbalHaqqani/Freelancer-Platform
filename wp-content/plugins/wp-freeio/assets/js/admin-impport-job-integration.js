(function ($) {
    ...
    WJBPAdminImportJobIntegrationCore.prototype = {
        ...
        init: function() {
            $('input[name=submit-cmb-indeed-job-import]').on('click', function() {
                ...
                $.ajax({
                    url: wp_freeio_iji_opts.ajaxurl,
                    type:'POST',
                    dataType: 'json',
                    data:  form.serialize()+"&action=wp_freeio_ajax_indeed_job_import"
                }).done(function(data) {
            ...
            $('input[name=submit-cmb-ziprecruiter-job-import]').on('click', function() {
                ...
                $.ajax({
                    ...
                    type:'POST',
                    dataType: 'json',
                    data:  form.serialize()+"&action=wp_freeio_ajax_ziprecruiter_job_import"
                }).done(function(data) {
                    form.removeClass('loading');
                    if ( data.status ) {
                        form.prepend( '<div class="alert alert-info">' + data.msg + '</div>' );
            ...
            $('input[name=submit-cmb-careerjet-job-import]').on('click', function() {
                ...
                $.ajax({
                    ...
                    type:'POST',
                    dataType: 'json',
                    data:  form.serialize()+"&action=wp_freeio_ajax_careerjet_job_import"
                }).done(function(data) {
                    form.removeClass('loading');
                    if ( data.status ) {
                        form.prepend( '<div class="alert alert-info">' + data.msg + '</div>' );
            ...
            $('input[name=submit-cmb-careerbuilder-job-import]').on('click', function() {
                ...
                $.ajax({
                    ...
                    type:'POST',
                    dataType: 'json',
                    data:  form.serialize()+"&action=wp_freeio_ajax_careerbuilder_job_import"
                }).done(function(data) {
                    form.removeClass('loading');
                    if ( data.status ) {
                        form.prepend( '<div class="alert alert-info">' + data.msg + '</div>' );
    ...
    $.wpfiAdminImportJobIntegrationCore = WJBPAdminImportJobIntegrationCore.prototype;

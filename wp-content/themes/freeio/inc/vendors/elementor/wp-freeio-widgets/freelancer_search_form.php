...
class Freeio_Elementor_Freelancer_Search_Form extends Elementor\Widget_Base {

	public function get_name() {
        return 'apus_element_freelancer_search_form';
    }

	public function get_title() {
        return esc_html__( 'Apus Freelancer Search Form', 'freeio' );
    }
	...
	protected function register_controls() {
        ...
        $fields = apply_filters( 'wp-freeio-default-freelancer-filter-fields', array() );
	...
	protected function render() {
        ...
        $search_page_url = WP_Freeio_Mixes::get_freelancers_page_url();

        freeio_load_select2();

        $filter_fields = apply_filters( 'wp-freeio-default-freelancer-filter-fields', array() );
        $instance = array();
            ...
            foreach ($search_fields as $item) {
                ...
                if ( $item['filter_field'] == 'title' ) {
                    if ($item['enable_autocompleate_search']) {
                        ...
                        $filter_field['add_class'] = 'apus-autocompleate-input autocompleate-freelancer';
                    }
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Freeio_Elementor_Freelancer_Search_Form );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Freelancer_Search_Form );
}